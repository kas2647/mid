resource "azurerm_virtual_machine_scale_set" "user01vmss2" {
name = "user01vmss2"
...
os_profile {
    ...
    admin_username = "kanglinux01"  ## VM 에 접속할 계정
...
ssh_keys {
    path = "/home/kanglinux01/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user01 등 
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user01-subnet2.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user01-bpepool2.id]
